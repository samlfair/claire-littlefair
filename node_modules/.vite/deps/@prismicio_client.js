...
var buildQueryURL = (endpoint, args) => {
  var _a;
  const _a2 = args, { predicates: predicates2 } = _a2, params = __objRest(_a2, ["predicates"]);
  const url = new URL(`documents/search`, `${endpoint}/`);
  if (predicates2) {
    for (const predicate2 of castArray(predicates2)) {
...
var appendPredicates = (objWithPredicates = {}, predicates2) => {
  return __spreadProps(__spreadValues({}, objWithPredicates), {
    predicates: [
      ...objWithPredicates.predicates || [],
      ...castArray(predicates2)
    ]
  });
};
...
var getAll = (cookieStore) => parse(cookieStore);
var getCookie = (name, cookieJar) => getAll(cookieJar)[name];
...
var createClient = (...args) => new Client(...args);
var Client = class {
  ...
  async query(predicates2, params) {
    const url = await this.buildQueryURL(__spreadProps(__spreadValues({}, params), { predicates: predicates2 }));
  ...
  async dangerouslyGetAll(params = {}) {
    var _a;
    const _a2 = params, { limit = Infinity } = _a2, actualParams = __objRest(_a2, ["limit"]);
    ...
    while ((!latestResult || latestResult.next_page) && documents.length < limit) {
      ...
      latestResult = await this.get(__spreadProps(__spreadValues({}, resolvedParams), { page }));
  ...
  async buildQueryURL(_a = {}) {
    ...
    return buildQueryURL(this.endpoint, __spreadProps(__spreadValues(__spreadValues({}, this.defaultParams), params), {
      ref,
  ...
  async graphQLFetch(input, init) {
    ...
    return await this.fetchFn(url.toString(), __spreadProps(__spreadValues({}, init), {
      headers
    }));
  }
...
var getEndpoint = getRepositoryEndpoint;
